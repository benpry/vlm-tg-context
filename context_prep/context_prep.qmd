...
yoked_cumul <- nested |>
  ...
  left_join(
    nested |> select(gameId, msg_trial_num = trialNum, messages, tangram),
...
random <- random_msg |>
  left_join(random_cumul, by = join_by(workerid, gameId, matcher_trialNum)) |>
  mutate(
    message = map_vec(messages, toJSON),
    condition = "random"
  ) |>
...
backward_cumul <- backward_nested |>
  ...
  left_join(
    backward_nested |>
      select(gameId, msg_trial_num = trialNum, messages, tangram),
    by = join_by(gameId, trialNum > msg_trial_num)
  ) |>
  group_by(gameId, trialNum) |>
  summarise(
...
wrong_within_nested <- nested |>
  ...
  left_join(
    nested |> select(gameId, repNum, messages, tangram),
